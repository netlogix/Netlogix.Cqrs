<?php
namespace Neos\Flow\Persistence\Doctrine\Migrations;

use Doctrine\Migrations\AbstractMigration;
use Doctrine\DBAL\Schema\Schema;

/**
 * Create command log table
 */
class Version20170127171026 extends AbstractMigration
{

	/**
	 * @return string
	 */
	public function getDescription(): string
	{
		return '';
	}

	/**
	 * @param Schema $schema
	 * @return void
	 */
	public function up(Schema $schema): void
	{
		$this->abortIf($this->connection->getDatabasePlatform()
				->getName() != 'postgresql', 'Migration can only be executed safely on "postgresql".');

		$this->addSql('COMMENT ON COLUMN netlogix_cqrs_log_commandlogentry.command IS \'(DC2Type:objectblob)\'');
		$this->addSql('COMMENT ON COLUMN netlogix_cqrs_log_commandlogentry.exception IS \'(DC2Type:objectblob)\'');
	}

	/**
	 * @param Schema $schema
	 * @return void
	 */
	public function down(Schema $schema): void
	{
		$this->abortIf($this->connection->getDatabasePlatform()
				->getName() != 'postgresql', 'Migration can only be executed safely on "postgresql".');

		$this->addSql('COMMENT ON COLUMN netlogix_cqrs_log_commandlogentry.command IS \'(DC2Type:commandobject)\'');
		$this->addSql('COMMENT ON COLUMN netlogix_cqrs_log_commandlogentry.exception IS \'(DC2Type:exceptionobject)\'');
	}
}
